...
public static class ShaderHelper
{
	...
	public static void Dispatch(ComputeShader cs, int numIterationsX, int numIterationsY = 1, int numIterationsZ = 1, int kernelIndex = 0)
	{
		...
		int numGroupsX = CeilToInt(numIterationsX / (float)threadGroupSizes.x);
		int numGroupsY = CeilToInt(numIterationsY / (float)threadGroupSizes.y);
		int numGroupsZ = CeilToInt(numIterationsZ / (float)threadGroupSizes.y);
